...
Alongside each scientific name, the app looks up an English common name
using the NBN Atlas's bulk species lookup service
(`species-ws.nbnatlas.org/species/lookup/bulk`). This is a separate request
from the main occurrence search, made once per search after the species
list comes back. If a species has no widely used common name (common for
many insects and invertebrates), or if the lookup service is unreachable,
the app shows "—" for that row rather than failing the whole search.
